document.addEventListener('DOMContentLoaded', () => {
    ...
    // This now contains the full list of 28 Dart questions.
    allQuestions = [
        ...
        },
        {
            ...
            explanation: "`name ?? 'stranger'` uses 'stranger' if `name` is null. `greet('')` uses empty string, printing 'Hello, !'."
        },
        {
            id: 'ws16_dart_q2',
            type: 'multiple-choice',
...
class Dog extends Animal {
  ...
  String toString() => '\$name the \$breed';
    ...
    ]; // End of allQuestions array definition
            ...
            currentQuestions.forEach((q, index) => {
                ...
                if (q.options && (q.type === 'multiple-choice' || q.type === 'code-snippet')) {
                    q.options.forEach((opt) => {
                        // Corrected: Use template literals for strings containing substitutions
                        questionHTML += `
                            <label>
    ...
    function handleSubmit() {
        ...
        currentQuestions.forEach((q, index) => {
            ...
            // Start of a new feedback block for each question
            detailedFeedbackHTML += `<div class="feedback-question-block">`;
            detailedFeedbackHTML += `<h4>Question ${index + 1}: ${escapeHTML(q.text)}</h4>`; // Question text as a heading
            ...
            let answered = false;
            let userAnswerDisplayText = ''; // To build a display string for user's answer

            // Variables for multiple-select evaluation, declared in the loop's scope
            ...
            if (q.type === 'multiple-choice' || q.type === 'code-snippet') {
                ...
                if (selectedOptionInput) {
                    answered = true;
                    const userAnswerValue = selectedOptionInput.value; // This is already HTML escaped
                ...
                selectedCheckboxes.forEach(cb => {
                    ...
                    selectedEscapedTexts.push(cb.value); // Value is already HTML escaped
                });
                ...
                // Build feedback status message for MS
                if (answered) {
                    if (ms_isPerfectScore) {
            ...
            // Apply styling to the question card in the main exam view (before it's hidden)
            if (questionCard) {
                ...
                if (explanationDiv) {
                    explanationDiv.style.display = 'block'; // Show explanation on the card itself
                }
            }
            ...
            if (index < currentQuestions.length - 1) { // Add a divider, but not after the last question
    ...
    // Simplified startExam for a single main mock
    function startExam() {
        ...
        // Removed the slice to 25, will use all questions from allQuestions array.
        // If you want to cap it, you can add:
        // if (currentQuestions.length > 25) { currentQuestions = currentQuestions.slice(0, 25); }
    ...
    if (restartButton) {
        restartButton.addEventListener('click', () => {
            ...
            if(examContainer) {
                examContainer.innerHTML = ''; // Clear rendered questions
            }
        });
    }
});